...
namespace Task4_2
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            con.GetConnection();   // Establish DB connection
            con.SelectQuery("SELECT * FROM dbo.Categories"); //Get data from DB
            con.GetDeleteUpdateQuery("UPDATE dbo.Categories SET Picture = 'unknown picture' WHERE CategoryID = 1"); // Update data in the table
            con.GetDeleteUpdateQuery("DELETE FROM dbo.Orders WHERE OrderID = 10248"); // Delete data in the table
            con.CallProcedure("Ten Most Expensive Products"); // Call function/procedure
            con.CloseConnection();
